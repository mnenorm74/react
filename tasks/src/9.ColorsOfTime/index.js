...
/**
 ...
 Используй ее!
 */
const CurrentTimeContext = React.createContext(new Timer());
...
class Top extends React.PureComponent {
    render() {
     ...
     {/*               {theme => (<Button value="← цвет" theme={theme} onClick={onPrevTheme}/>)}
                    {theme => (<Button value="цвет →" theme={theme} onClick={onNextTheme}/>)}*/}
                </ChangeThemeContext.Consumer>
            </div>
        );
    }
